...
function setBackground() {
  ...
  const isArticlePage = !!(bodyWrap && bodyWrap.classList.contains('post'));
  console.log('header element:', h);

  if (h) {
    if (isArticlePage) {
      // Article pages: keep cool blue background
      h.style.setProperty('background', '#081828', 'important');
      h.style.setProperty('background-image', 'none', 'important');
      console.log('Article page: cool blue background applied');
    } else if (h.classList.contains('full_page')) {
      // Homepage: transparent to let GLB 3D canvas show through
      h.style.setProperty('background', 'transparent', 'important');
      h.style.setProperty('background-image', 'none', 'important');
      h.style.setProperty('z-index', '10', 'important');
      console.log('Homepage: transparent background for GLB');
    } else {
      // Archive/Tag/Category pages: use banner.jpg with larger header
      h.style.setProperty('background-image', 'url(/img/banner.jpg)', 'important');
      h.style.setProperty('background-size', 'cover', 'important');
      h.style.setProperty('background-position', 'center', 'important');
      h.style.setProperty('z-index', '10', 'important');
      h.style.setProperty('height', '70vh', 'important');

      // Center the page title vertically in the taller header
      var pageInfo = h.querySelector('#page-site-info');
      if (pageInfo) {
        pageInfo.style.setProperty('top', '50%', 'important');
        pageInfo.style.setProperty('transform', 'translateY(-50%)', 'important');
      }
      console.log('Non-home page: banner.jpg background applied (70vh)');
    }
  } else {
...
function fixArticlePageBackground() {
  ...
  if (bodyWrap && bodyWrap.classList.contains('post')) {
    ...
    console.log('Article page background applied');
